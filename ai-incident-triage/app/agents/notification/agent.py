...
def run_notification_agent(
    ...
    try:
        ...
        # LLM-drafted prose is untrusted free-form content: strip disallowed
        # tags/attributes before it leaves the system.
        ...
        # Fixed-template output is trusted static HTML filled with autoescaped,
        # validated report values -- it must keep its inline CSS (table layout, badges)
        # for Outlook-compatible delivery, so the LLM allow-list sanitizer is not
        # applied here (it would strip `style=` attributes and break the styled layout).
