...
def test_database_timeout_resolves_and_notifies() -> None:
    result = runner.invoke(
        app,
        [str(INCIDENTS / "database_timeout.json")],
    )
...
def test_unresolved_incident() -> None:
    ...
    result = runner.invoke(
        app,
        [str(INCIDENTS / "unmatched-no-telemetry.json")],
    )
    assert result.exit_code == 0
...
def test_missing_file_exits_nonzero() -> None:
    result = runner.invoke(
        app,
        ["data/incidents/does_not_exist.json"],
    )
...
def test_missing_title_exits_nonzero(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, [str(no_title)])
    assert result.exit_code == 1
