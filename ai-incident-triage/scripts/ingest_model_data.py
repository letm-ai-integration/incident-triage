...
def _ingest_runbooks() -> int:
    ...
    # Per-service runbooks live under ``knowledge_base/runbooks/`` (plus legacy
    # ``runbooks/`` at the repo root for backwards-compat), display-name-keyed
    # and used verbatim by the name-keyed runbook resolver.
    runbooks_dirs = [repo_root / "knowledge_base" / "runbooks", repo_root / "runbooks"]
    seen: set[Path] = set()
    for runbooks_dir in runbooks_dirs:
